...
/**
 * 화면 녹화 권한(MediaProjection) 팝업을 띄우기 위한 투명 액티비티.
 * "허용" 즉시 결과를 OverlayService 로 넘겨 그 순간부터 매크로+영상 녹화를 시작한다.
 * (녹화 버튼 탭·허용 탭은 이 시점 이전이라 매크로에 안 찍힘)
 */
class ProjectionActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        runCatching { startActivityForResult(mpm.createScreenCaptureIntent(), REQ) }
            .onFailure { forward(Intent(this, OverlayService::class.java).apply { action = OverlayService.ACTION_START_MACRO_ONLY }); finish() }
    }
